...
public static class DialogService
{
    ...
    public static string SelectFileDialog(string[] files)
    {
        var selectedIndex = ListService.ListDialog("Select a file", files.ToList());
        var selectedFile = selectedIndex.Count > 0 ? selectedIndex.First().Value : "";
        return string.IsNullOrEmpty(selectedFile) ? "" : Path.Combine(Environment.CurrentDirectory, selectedFile);
    }
